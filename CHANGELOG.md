...
**8 项功能/性能优化合入与 MCP Hub ^0.3.3 升级 🚀**

本次更新合入了 8 项关键交互与性能优化（Opt 1–8），并将 `@at-series/mcp-hub` 依赖升级至最新版本 `^0.3.3`。

### ⚡ 功能与性能优化 (Features & Performance)
- **草稿保存与发布解耦**：`nacos-draft` 支持纯本地保存编辑，将本地文件保存与发布至 Nacos 流程解耦。
- **实例表单状态保持**：解决实例编辑 Webview Tab 切换到后台/隐藏时丢失未保存字段输入的问题。
- **连接池刷新性能提升**：写操作后触发刷新时不再频繁强制清空 Client Pool，大幅提升响应速度。
- **服务列表过滤搜索**：在服务树视图顶部添加按服务名模糊过滤功能与清除过滤按钮。
- **实例上下文菜单增强**：在实例节点右键菜单中增加「编辑实例」、「上下线/删除实例」、「查看所属集群状态」快捷入口。
- **命名空间与分组快捷新建配置**：在命名空间和分组节点右键菜单新增「New Configuration」命令并自动带入上下文。
- **MCP 证书与集群监控优化**：并行请求集群节点与指标；MCP 路径下适配非交互式证书验证器（TOFU）。
- **MCP 配置卸载命令**：新增 `atNacos.uninstallMcpConfig` 命令并提供 3 态安装反馈，修正 AK/SK 文档说明。

### 📦 依赖升级 (Dependencies)
- 升级 `@at-series/mcp-hub` 到 `^0.3.3`。
