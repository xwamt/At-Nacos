...
- [x] 配置节点能打开历史面板，列出版本（夹具验证；真机为空 —— 真机跑到的是空态那一句）
- [x] 能与历史版本 diff，两侧内容正确、语法高亮正确 —— 两侧都是 `nacos:` 地址，走同一条 M2 的渲染路径，语言模式由 `configLanguageId` 决定；真机验到的是「不存在的 nid 给出的是可读的一句话」
- [x] 能跨环境 diff 同名配置（**真机验证**：`cl-parent` 与 `damon` 下的 `sentinel-cl-gateway`，858 vs 677 字节，两侧确实不同）
- [x] 目标环境无此配置时给出明确提示而非空白栏（**真机验证**）
- [x] 配置监听者可查（夹具验证；真机上没有任何客户端长轮询，验到的是空态与「配置当前 md5 读得到」）
- [x] 服务订阅者可查（**真机验证**：12 个服务全部有行，其中一个两行）
- [x] `openPanels` 已提取为共享模块（`src/webview/openPanels.ts`，集群面板一并迁入）
- [x] 架构文档 §14 记录历史与监听者未经真机验证及原因（§14.9 ⑤ 与 ㉛）
