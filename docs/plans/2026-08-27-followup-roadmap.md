...
> **Status:** 分析文档。对照「1–8 项体验/性能优化」合入后的代码盘点剩余缺口（该批次约 1980 单测通过 / 33 live 跳过）。
>
> **若 1–8 尚未合入 main：** 下列能力仍应先做，勿与本文后半的「剩余项」混淆——写路径不清整个连接池、Ctrl+S 不发布、实例表单保活、新建配置、MCP 卸载与诚实提示、连接节点右键、服务名过滤、MCP 并行拉集群 + 非交互证书。
>
